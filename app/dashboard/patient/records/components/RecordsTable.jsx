...
import { getMyDocuments, fetchProfile } from "@/services/patient";
...
function getFileType(record) {
  const type = record.file_type || record.fileType;
  if (type) return type.toLowerCase().replace(".", "");
  const name = record.file_name || record.fileName || record.title || record.presigned_url || record.url || record.downloadUrl || "";
...
export default function RecordsTable() {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      const data = await getMyDocuments(patientId);
      ...
      setRecords(Array.from(uniqueFiles.values()).sort((a, b) =>
        new Date(b.uploaded_at || b.uploadedAt || b.created_at) - new Date(a.uploaded_at || a.uploadedAt || a.created_at)
      ));
    } catch (err) {
  ...
  const filtered = records.filter(record => {
    ...
    // Month filter
    if (monthFilter !== "All") {
      const dateStr = record.uploaded_at || record.uploadedAt || record.upload_date || record.created_at;
              ...
              {filtered.map((record) => (
                <tr key={record.id || Math.random()}>
                  <td style={{ whiteSpace: "nowrap" }}>
                    {record.uploaded_at || record.uploadedAt || record.upload_date || record.created_at
                      ? new Date(record.uploaded_at || record.uploadedAt || record.upload_date || record.created_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
                      : "—"}
                  </td>
                  <td>{record.title || record.file_name || record.fileName || "Untitled Document"}</td>
                  <td><FileTypeBadge record={record} /></td>
                  <td>
                    {(() => {
                      const hasUrl = record.presigned_url || record.downloadUrl || record.download_url || record.url || record.file_url;
                      return (
